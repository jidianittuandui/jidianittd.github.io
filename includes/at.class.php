...
/*class AT {
		...
		function __construct($AT_host,$AT_user,$AT_pass,$AT_name,$AT_port){

		$this->link = @mysql_connect($AT_host.':'.$AT_port, $AT_user, $AT_pass);
            
		if (!$this->link) die('Connect Error (' . mysql_errno() . ') '.mysql_error());
            
			mysql_select_db($AT_name, $this->link) or die(mysql_error($this->link));
...
if(defined('SQLITE')==true){
	class AT {
		...
		function __construct($AT_file){
			global $siteurl;
		$this->link = new PDO('sqlite:'.ROOT.'includes/sqlite/'.$AT_file.'.AT');
...
elseif(extension_loaded('mysqli') && $nomysqli==false) {
    class AT {
        ...
        function __construct($AT_host,$AT_user,$AT_pass,$AT_name,$AT_port){
            ...
            $this->link = mysqli_connect($AT_host, $AT_user, $AT_pass, $AT_name, $AT_port);
            
            if (!$this->link) die('Connect Error (' . mysqli_connect_errno() . ') '.mysqli_connect_error());
            
            //mysqli_select_AT($this->link, $AT_name) or die(mysqli_error($this->link));
